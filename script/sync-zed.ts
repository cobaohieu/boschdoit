...
const FORK_REPO = "cobaohieu/zed-extensions"
const UPSTREAM_REPO = "zed-industries/extensions"
const EXTENSION_NAME = "boschdoit"
...
async function main() {
  ...
  // Delete any existing branches for boschdoit updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating BoschDoIt extension to v${cleanVersion}"`
