...
import { createBoschdoitClient } from "@boschdoit-ai/sdk"
...
const { client, server } = createBoschdoit()
...
try {
  ...
  await assertBoschdoitConnected()
  ...
  console.log("boschdoit session", session.id)
  if (shareId) {
...
function createBoschdoit() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`boschdoit`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createBoschdoitClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/boschdoit|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/boschdoit` or `/oc`")
  }
}
...
async function assertBoschdoitConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to boschdoit server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.boschdoit.ai" : "https://boschdoit.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.boschdoit.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("boschdoit-github-action")
    response = await fetch("https://api.boschdoit.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/boschdoit" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/boschdoit") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to boschdoit...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "boschdoit-agent[bot]"`
  await $`git config --global user.email "boschdoit-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `boschdoit/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/boschdoit-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[boschdoit session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
