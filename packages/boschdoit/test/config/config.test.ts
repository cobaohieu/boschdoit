...
const managedConfigDir = process.env.BOSCHDOIT_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "boschdoit.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "boschdoit.json") {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://boschdoit.ai/config.json",
        model: "test/model",
        username: "testuser",
      })
    },
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "boschdoit.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "boschdoit.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .boschdoit directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const boschdoitDir = path.join(dir, ".boschdoit")
      await fs.mkdir(boschdoitDir, { recursive: true })
      const agentDir = path.join(boschdoitDir, "agent")
...
test("loads agents from .boschdoit/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const boschdoitDir = path.join(dir, ".boschdoit")
      await fs.mkdir(boschdoitDir, { recursive: true })

      const agentsDir = path.join(boschdoitDir, "agents")
...
test("loads commands from .boschdoit/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const boschdoitDir = path.join(dir, ".boschdoit")
      await fs.mkdir(boschdoitDir, { recursive: true })

      const commandDir = path.join(boschdoitDir, "command")
...
test("loads commands from .boschdoit/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const boschdoitDir = path.join(dir, ".boschdoit")
      await fs.mkdir(boschdoitDir, { recursive: true })

      const commandsDir = path.join(boschdoitDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "boschdoit.json"),
        JSON.stringify({ $schema: "https://boschdoit.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "boschdoit.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .boschdoit config
      const projectDir = path.join(dir, "project")
      const boschdoitDir = path.join(projectDir, ".boschdoit")
      await fs.mkdir(boschdoitDir, { recursive: true })
      ...
      // Local .boschdoit config with different plugins
      await Bun.write(
        path.join(boschdoitDir, "boschdoit.json"),
        JSON.stringify({
          $schema: "https://boschdoit.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .boschdoit config with some overlapping plugins
      await Bun.write(
        path.join(boschdoitDir, "boschdoit.json"),
        JSON.stringify({
          $schema: "https://boschdoit.ai/config.json",
...
// Note: preload.ts sets BOSCHDOIT_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .boschdoit config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .boschdoit directory config enables it
      const boschdoitDir = path.join(dir, ".boschdoit")
      await fs.mkdir(boschdoitDir, { recursive: true })
      await Bun.write(
        path.join(boschdoitDir, "boschdoit.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/boschdoit")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/boschdoit")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-boschdoit@2.4.3")).toBe("oh-my-boschdoit")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@boschdoit/plugin@2.0.0")).toBe("@boschdoit/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-boschdoit@2.4.3", "file:///project/.boschdoit/plugin/oh-my-boschdoit.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.boschdoit/plugin/oh-my-boschdoit.js")
  })
  ...
  test("local plugin directory overrides global boschdoit.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const boschdoitDir = path.join(projectDir, ".boschdoit")
        const pluginDir = path.join(boschdoitDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("BOSCHDOIT_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["BOSCHDOIT_DISABLE_PROJECT_CONFIG"]
    process.env["BOSCHDOIT_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["BOSCHDOIT_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["BOSCHDOIT_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .boschdoit/ directories when flag is set", async () => {
    const originalEnv = process.env["BOSCHDOIT_DISABLE_PROJECT_CONFIG"]
    process.env["BOSCHDOIT_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .boschdoit directory with a command
          const boschdoitDir = path.join(dir, ".boschdoit", "command")
          await fs.mkdir(boschdoitDir, { recursive: true })
          await Bun.write(path.join(boschdoitDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .boschdoit should NOT be in directories list
          const hasProjectBoschdoit = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectBoschdoit).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["BOSCHDOIT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["BOSCHDOIT_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["BOSCHDOIT_CONFIG_DIR"]
      process.env["BOSCHDOIT_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["BOSCHDOIT_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["BOSCHDOIT_CONFIG_DIR"]
      } else {
        process.env["BOSCHDOIT_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("BOSCHDOIT_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["BOSCHDOIT_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["BOSCHDOIT_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["BOSCHDOIT_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["BOSCHDOIT_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from BOSCHDOIT_CONFIG_DIR, not project
