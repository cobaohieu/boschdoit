...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const boschdoitModel = {
      ...openaiModel,
      providerID: "boschdoit",
      api: {
        id: "boschdoit-test",
        url: "https://api.boschdoit.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, boschdoitModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.boschdoit?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.boschdoit?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          boschdoit: { itemId: "msg_boschdoit" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              boschdoit: { itemId: "msg_boschdoit_part" },
    ...
    expect(result[0].providerOptions?.boschdoit?.itemId).toBe("msg_boschdoit")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.boschdoit?.itemId).toBe("msg_boschdoit_part")
