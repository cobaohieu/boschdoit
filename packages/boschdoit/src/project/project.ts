...
import { fn } from "@boschdoit-ai/util/fn"
  ...
  export async function fromDirectory(directory: string) {
    ...
    const { id, sandbox, worktree, vcs } = await iife(async () => {
      ...
      if (git) {
        ...
        let id = await Bun.file(path.join(git, "boschdoit"))
          .text()
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.BOSCHDOIT_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            void Bun.file(path.join(git, "boschdoit"))
              .write(id)
              .catch(() => undefined)
          }
        }
    ...
    if (Flag.BOSCHDOIT_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
