...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run boschdoit",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for boschdoit session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found boschdoit session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`boschdoit import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting boschdoit...")
        UI.println()

        // Launch boschdoit TUI with session ID if available
        const { spawn } = await import("child_process")
        const boschdoitArgs = sessionId ? ["-s", sessionId] : []
        const boschdoitProcess = spawn("boschdoit", boschdoitArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          boschdoitProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`boschdoit exited with code ${code}`))
          })
          boschdoitProcess.on("error", reject)
        })
      },
    })
  },
