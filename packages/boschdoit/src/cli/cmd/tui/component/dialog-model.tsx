...
export function useConnected() {
  const sync = useSync()
  return createMemo(() =>
    sync.data.provider.some((x) => x.id !== "boschdoit" || Object.values(x.models).some((y) => y.cost?.input !== 0)),
  )
}
...
export function DialogModel(props: { providerID?: string }) {
  ...
  const options = createMemo(() => {
          ...
          return [
            {
              ...
              category: "Favorites",
              disabled: provider.id === "boschdoit" && model.id.includes("-nano"),
              footer: model.cost?.input === 0 && provider.id === "boschdoit" ? "Free" : undefined,
              onSelect: () => {
                dialog.clear()
                local.model.set(
                  {
    ...
    const providerOptions = pipe(
      sync.data.provider,
      sortBy(
        (provider) => provider.id !== "boschdoit",
        ...
        pipe(
          ...
          map(([model, info]) => {
            ...
            return {
              ...
              disabled: provider.id === "boschdoit" && model.includes("-nano"),
              footer: info.cost?.input === 0 && provider.id === "boschdoit" ? "Free" : undefined,
              onSelect() {
                dialog.clear()
                local.model.set(
                  {
