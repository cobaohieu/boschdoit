...
import { createBoschdoitClient, type BoschdoitClient } from "@boschdoit-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run boschdoit with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running boschdoit server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: BoschdoitClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createBoschdoitClient({ baseUrl: args.attach })
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.BOSCHDOIT_AUTO_SHARE || args.share)) {
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createBoschdoitClient({ baseUrl: "http://boschdoit.internal", fetch: fetchFn })
