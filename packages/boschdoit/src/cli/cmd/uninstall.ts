...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall boschdoit and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall BoschDoIt")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g boschdoit-ai",
      pnpm: "pnpm uninstall -g boschdoit-ai",
      bun: "bun remove -g boschdoit-ai",
      yarn: "yarn global remove boschdoit-ai",
      brew: "brew uninstall boschdoit",
      choco: "choco uninstall boschdoit",
      scoop: "scoop uninstall boschdoit",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall boschdoit -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using BoschDoIt!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# boschdoit") || content.includes(".boschdoit/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".boschdoit/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".boschdoit/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".boschdoit"))
    ) {
      continue
    }
