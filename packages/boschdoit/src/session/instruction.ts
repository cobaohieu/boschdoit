...
function globalFiles() {
  ...
  if (!Flag.BOSCHDOIT_DISABLE_CLAUDE_CODE_PROMPT) {
    files.push(path.join(os.homedir(), ".claude", "CLAUDE.md"))
  }
  if (Flag.BOSCHDOIT_CONFIG_DIR) {
    files.push(path.join(Flag.BOSCHDOIT_CONFIG_DIR, "AGENTS.md"))
  }
  return files
}
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.BOSCHDOIT_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no BOSCHDOIT_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.BOSCHDOIT_CONFIG_DIR, Flag.BOSCHDOIT_CONFIG_DIR).catch(() => [])
}
