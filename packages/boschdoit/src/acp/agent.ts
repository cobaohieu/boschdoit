...
import type { Event, BoschdoitClient, SessionMessageResponse } from "@boschdoit-ai/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: BoschdoitClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `boschdoit auth login` in the terminal",
        name: "Login with boschdoit",
        id: "boschdoit-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // BoschDoIt stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const boschdoitProvider = providers.find((p) => p.id === "boschdoit")
    if (boschdoitProvider) {
      if (boschdoitProvider.models["big-pickle"]) {
        return { providerID: "boschdoit", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(boschdoitProvider.models))
      if (best) {
        return {
