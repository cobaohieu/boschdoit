...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for boschdoit.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal boschdoit sessions
...
boschdoit acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via boschdoit's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | boschdoit acp
```
...
### Mapping to BoschDoIt

ACP sessions map cleanly to boschdoit's internal session model:
