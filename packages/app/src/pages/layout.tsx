...
import { Avatar } from "@boschdoit-ai/ui/avatar"
import { ResizeHandle } from "@boschdoit-ai/ui/resize-handle"
...
import { InlineInput } from "@boschdoit-ai/ui/inline-input"
import { Tooltip, TooltipKeybind } from "@boschdoit-ai/ui/tooltip"
import { HoverCard } from "@boschdoit-ai/ui/hover-card"
import { MessageNav } from "@boschdoit-ai/ui/message-nav"
import { DropdownMenu } from "@boschdoit-ai/ui/dropdown-menu"
import { Collapsible } from "@boschdoit-ai/ui/collapsible"
import { DiffChanges } from "@boschdoit-ai/ui/diff-changes"
import { Spinner } from "@boschdoit-ai/ui/spinner"
import { Dialog } from "@boschdoit-ai/ui/dialog"
import { getFilename } from "@boschdoit-ai/util/path"
import { Session, type Message, type TextPart } from "@boschdoit-ai/sdk/v2/client"
...
import { showToast, Toast, toaster } from "@boschdoit-ai/ui/toast"
import { useGlobalSDK } from "@/context/global-sdk"
import { useNotification } from "@/context/notification"
import { usePermission } from "@/context/permission"
import { Binary } from "@boschdoit-ai/util/binary"
import { retry } from "@boschdoit-ai/util/retry"
import { playSound, soundSrc } from "@/utils/sound"
import { Worktree as WorktreeState } from "@/utils/worktree"
import { agentColor } from "@/utils/agent"

import { useDialog } from "@boschdoit-ai/ui/context/dialog"
import { useTheme, type ColorScheme } from "@boschdoit-ai/ui/theme"
...
export default function Layout(props: ParentProps) {
  ...
  const deepLinkEvent = "boschdoit:deep-link"

  const parseDeepLink = (input: string) => {
    if (!input.startsWith("boschdoit://")) return
  ...
  const drainDeepLinks = () => {
    const pending = window.__BOSCHDOIT__?.deepLinks ?? []
    if (pending.length === 0) return
    if (window.__BOSCHDOIT__) window.__BOSCHDOIT__.deepLinks = []
  ...
  const ProjectIcon = (props: { project: LocalProject; class?: string; notify?: boolean }): JSX.Element => {
    ...
    const boschdoit = "4b0ea68d7af9a6031a7ffda7ad66e0cb83315750"
    ...
    return (
      <div class={`relative size-8 shrink-0 rounded ${props.class ?? ""}`}>
        <div class="size-full rounded overflow-clip">
          <Avatar
            fallback={name()}
            src={props.project.id === boschdoit ? "https://boschdoit.ai/favicon.svg" : props.project.icon?.override}
              ...
              <div class="overflow-y-auto max-h-72 h-full">
                <MessageNav
                  ...
                  onMessageSelect={(message) => {
                    if (!isActive()) {
                      sessionStorage.setItem("boschdoit.pendingMessage", `${props.session.id}|${message.id}`)
          ...
          <div class="shrink-0 w-full pt-3 pb-3 flex flex-col items-center gap-2">
            ...
            <Tooltip placement={sidebarProps.mobile ? "bottom" : "right"} value={language.t("sidebar.help")}>
              <IconButton
                ...
                variant="ghost"
                size="large"
                onClick={() => platform.openLink("https://boschdoit.ai/desktop-feedback")}
