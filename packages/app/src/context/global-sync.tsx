import {
  ...
  createBoschdoitClient,
} from "@boschdoit-ai/sdk/v2/client"
...
function createGlobalSync() {
  ...
  const sdkCache = new Map<string, ReturnType<typeof createBoschdoitClient>>()
      ...
      retry(() =>
        globalSDK.client.project.list().then(async (x) => {
          const projects = (x.data ?? [])
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("boschdoit-test"))
            .slice()
            .sort((a, b) => a.id.localeCompare(b.id))
