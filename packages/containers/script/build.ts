...
const reg = process.env.REGISTRY ?? "ghcr.io/cobaohieu"
...
const setup = async () => {
  ...
  if (list.includes("boschdoit")) {
    await $`docker buildx use boschdoit`
    return
  }
  await $`docker buildx create --name boschdoit --use`
}
