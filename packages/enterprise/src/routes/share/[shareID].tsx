import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@boschdoit-ai/sdk/v2"
import { SessionTurn } from "@boschdoit-ai/ui/session-turn"
import { SessionReview } from "@boschdoit-ai/ui/session-review"
import { DataProvider } from "@boschdoit-ai/ui/context"
import { DiffComponentProvider } from "@boschdoit-ai/ui/context/diff"
import { CodeComponentProvider } from "@boschdoit-ai/ui/context/code"
import { WorkerPoolProvider } from "@boschdoit-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createEffect, createMemo, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@boschdoit-ai/ui/logo"
import { IconButton } from "@boschdoit-ai/ui/icon-button"
import { ProviderIcon } from "@boschdoit-ai/ui/provider-icon"
import { createDefaultOptions } from "@boschdoit-ai/ui/pierre"
import { iife } from "@boschdoit-ai/util/iife"
...
import { Tabs } from "@boschdoit-ai/ui/tabs"
import { MessageNav } from "@boschdoit-ai/ui/message-nav"
import { preloadMultiFileDiff, PreloadMultiFileDiffResult } from "@pierre/diffs/ssr"
import { Diff as SSRDiff } from "@boschdoit-ai/ui/diff-ssr"
import { clientOnly } from "@solidjs/start"
import { type IconName } from "@boschdoit-ai/ui/icons/provider"
import { Meta, Title } from "@solidjs/meta"
import { Base64 } from "js-base64"

const ClientOnlyDiff = clientOnly(() => import("@boschdoit-ai/ui/diff").then((m) => ({ default: m.Diff })))
const ClientOnlyCode = clientOnly(() => import("@boschdoit-ai/ui/code").then((m) => ({ default: m.Code })))
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@boschdoit-ai/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/boschdoit-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | BoschDoIt</Title>
              </Show>
              <Meta name="description" content="boschdoit - The AI coding agent built for the terminal." />
                          ...
                          <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                            <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                              ...
                              <div class="flex gap-3 items-center">
                                <IconButton
                                  as={"a"}
                                  href="https://github.com/cobaohieu/boschdoit"
                                  target="_blank"
                                  icon="github"
                                  variant="ghost"
                                />
                                <IconButton
                                  as={"a"}
                                  href="https://boschdoit.ai/discord"
                                  target="_blank"
                                  icon="discord"
                                  variant="ghost"
                                />
