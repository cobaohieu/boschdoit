...
import { AppBaseProviders, AppInterface, PlatformProvider, Platform } from "@boschdoit-ai/app"
...
import { Splash } from "@boschdoit-ai/ui/logo"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__BOSCHDOIT__ ??= {}
  const pending = window.__BOSCHDOIT__.deepLinks ?? []
  window.__BOSCHDOIT__.deepLinks = [...pending, ...urls]
...
const createPlatform = (password: Accessor<string | null>): Platform => ({
    ...
    await Promise.resolve()
      .then(() => {
        const notification = new Notification(title, {
          body: description ?? "",
          icon: "https://boschdoit.ai/favicon-96x96-v3.png",
        })
  ...
  fetch: (input, init) => {
    ...
    const addHeader = (headers: Headers, password: string) => {
      headers.append("Authorization", `Basic ${btoa(`boschdoit:${password}`)}`)
    }
      ...
      <AppBaseProviders>
        <ServerGate>
          {(data) => {
            ...
            window.__BOSCHDOIT__ ??= {}
            window.__BOSCHDOIT__.serverPassword = data().password ?? undefined
