...
const SETTINGS_STORE: &str = "boschdoit.settings.dat";
...
fn get_sidecar_port() -> u32 {
    option_env!("BOSCHDOIT_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("BOSCHDOIT_PORT").ok())
...
fn spawn_sidecar(app: &AppHandle, hostname: &str, port: u32, password: &str) -> CommandChild {
    ...
    .env("BOSCHDOIT_SERVER_USERNAME", "boschdoit")
    .env("BOSCHDOIT_SERVER_PASSWORD", password)
    .spawn()
    .expect("Failed to spawn boschdoit");
...
async fn check_server_health(url: &str, password: Option<&str>) -> bool {
    ...
    if let Some(password) = password {
        req = req.basic_auth("boschdoit", Some(password));
    }
            ...
            let window_builder = WebviewWindowBuilder::from_config(&app, config)
                ...
                .initialization_script(format!(
                    r#"
                      ...
                      window.__BOSCHDOIT__.updaterEnabled = {updater_enabled};
                    "#
                ));
...
async fn spawn_local_server(
    ...
    loop {
        if timestamp.elapsed() > Duration::from_secs(30) {
            break Err(format!(
                "Failed to spawn BoschDoIt Server. Logs:\n{}",
