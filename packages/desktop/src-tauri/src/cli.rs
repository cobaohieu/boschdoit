...
const CLI_INSTALL_DIR: &str = ".boschdoit/bin";
const CLI_BINARY_NAME: &str = "boschdoit";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("boschdoit-install.sh");
...
pub fn create_command(app: &tauri::AppHandle, args: &str) -> Command {
    ...
    return app
        .shell()
        .sidecar("boschdoit-cli")
        .unwrap()
        .args(args.split_whitespace())
        .env("BOSCHDOIT_EXPERIMENTAL_ICON_DISCOVERY", "true")
        .env("BOSCHDOIT_EXPERIMENTAL_FILEWATCHER", "true")
        .env("BOSCHDOIT_CLIENT", "desktop")
