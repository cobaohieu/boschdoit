...
There are two types of agents in BoschDoIt; primary agents and subagents.
...
BoschDoIt comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
BoschDoIt comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

BoschDoIt comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `boschdoit.json` config file:
...
- Global: `~/.config/boschdoit/agents/`
- Per-project: `.boschdoit/agents/`

```markdown title="~/.config/boschdoit/agents/review.md"
---
...
If no temperature is specified, BoschDoIt uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global BoschDoIt config and the project specific config.
...
The model ID in your BoschDoIt config uses the format `provider/model-id`. For example, if you're using [BoschDoIt Zen](/docs/zen), you would use `boschdoit/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="boschdoit.json" {3-6,9-12}
{
  "$schema": "https://boschdoit.ai/config.json",
  "tools": {
    "write": true,
...
```json title="boschdoit.json" {3-5,8-10}
{
  "$schema": "https://boschdoit.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `boschdoit models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/cobaohieu/boschdoit).
:::
...
```markdown title="~/.config/boschdoit/agents/docs-writer.md"
---
...
```markdown title="~/.config/boschdoit/agents/security-auditor.md"
---
