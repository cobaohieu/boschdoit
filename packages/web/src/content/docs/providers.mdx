---
title: Providers
description: Using any LLM provider in BoschDoIt.
---

import config from "../../../config.mjs"
export const console = config.console

BoschDoIt uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your BoschDoIt config.
...
in `~/.local/share/boschdoit/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your BoschDoIt
config.
...
BoschDoIt Zen is a list of models provided by the BoschDoIt team that have been
tested and verified to work well with BoschDoIt. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with BoschDoIt Zen.
:::

1. Run the `/connect` command in the TUI, select boschdoit, and head to [boschdoit.ai/auth](https://boschdoit.ai/auth).
...
It works like any other provider in BoschDoIt and is completely optional to use.
...
To use Amazon Bedrock with BoschDoIt:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running boschdoit:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY boschdoit

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile boschdoit

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX boschdoit
   ```
   ...
   For project-specific or persistent configuration, use `boschdoit.json`:

   ```json title="boschdoit.json"
   {
     "$schema": "https://boschdoit.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Using your Claude Pro/Max subscription in BoschDoIt is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for boschdoit to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX boschdoit
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX boschdoit
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your boschdoit config.
...
to store token in boschdoit auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
BoschDoIt uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock BoschDoIt
to only use your own GitLab-hosted instance, add the following to your
`boschdoit.json` file. It is also recommended to disable session sharing.
...
More documentation on [boschdoit-gitlab-auth](https://www.npmjs.com/package/@gitlab/boschdoit-gitlab-auth) homepage.

##### Configuration

Customize through `boschdoit.json`:
...
{
  "$schema": "https://boschdoit.ai/config.json",
  "plugin": ["@gitlab/boschdoit-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with boschdoit:
...
To use Google Vertex AI with BoschDoIt:
...
2. Set the required environment variables:
   ...
   Set them while running boschdoit.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id boschdoit
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through boschdoit, you can always configure it yourself.
...
```jsonc title="~/.config/boschdoit/boschdoit.jsonc"
{
  "$schema": "https://boschdoit.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [boschdoit-helicone-session](https://github.com/H2Shami/boschdoit-helicone-session) plugin to automatically log each BoschDoIt conversation as a session in Helicone.

```bash
npm install -g boschdoit-helicone-session
```
...
```json title="boschdoit.json"
{
  "plugin": ["boschdoit-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each BoschDoIt conversation listed as a separate session.
...
You can configure boschdoit to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="boschdoit.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://boschdoit.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure boschdoit to use local models through LM Studio.

```json title="boschdoit.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://boschdoit.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure boschdoit to use local models through Ollama.

```json title="boschdoit.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://boschdoit.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with BoschDoIt:
...
3. Copy the API key for use in BoschDoIt.
...
6. **Important**: Before using cloud models in BoschDoIt, you must pull the model information locally:
...
BoschDoIt Zen is a list of tested and verified models provided by the BoschDoIt team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>BoschDoIt Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **BoschDoIt Zen**.

   ```txt
   /connect
   ```

3. Enter your BoschDoIt API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your boschdoit config.
...
5. You can also customize them through your boschdoit config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' boschdoit
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group boschdoit
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Boschdoit:
...
You can also customize models through your boschdoit config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with boschdoit. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in boschdoit.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `boschdoit.json` file in your project directory:

   ```json title="boschdoit.json" ""myprovider"" {5-15}
   {
     "$schema": "https://boschdoit.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="boschdoit.json" {9,11,17-20}
{
  "$schema": "https://boschdoit.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow BoschDoIt to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `boschdoit auth list` to see if the credentials
...
2. For custom providers, check the boschdoit config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your boschdoit config.
