---
title: SDK
description: Type-safe JS client for boschdoit server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The boschdoit JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control boschdoit programmatically.
...
Create an instance of boschdoit:

```javascript
import { createBoschdoit } from "@boschdoit-ai/sdk"

const { client } = await createBoschdoit()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `boschdoit.json`, but you can override or add configuration inline:
...
console.log(`Server running at ${boschdoit.server.url}`)
...
If you already have a running instance of boschdoit, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@boschdoit-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*boschdoit" },
})
