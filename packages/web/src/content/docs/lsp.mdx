---
title: LSP Servers
description: BoschDoIt integrates with your LSP servers.
---

BoschDoIt integrates with your Language Server Protocol (LSP) to help the LLM interact with your codebase. It uses diagnostics to provide feedback to the LLM.

---

## Built-in

BoschDoIt comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `BOSCHDOIT_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When boschdoit opens a file, it:
...
You can customize LSP servers through the `lsp` section in your boschdoit config.
