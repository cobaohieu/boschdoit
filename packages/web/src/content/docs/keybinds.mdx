...
BoschDoIt has a list of keybinds that you can customize through the BoschDoIt config.
...
BoschDoIt uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The BoschDoIt desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `boschdoit.json`.
