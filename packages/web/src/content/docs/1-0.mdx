...
description: What's new in BoschDoIt 1.0.
---

BoschDoIt 1.0 is a complete rewrite of the TUI.
...
The new TUI works like the old one since it connects to the same boschdoit server.
...
version. However some older versions of BoschDoIt always grab latest.

To upgrade manually, run

```bash
$ boschdoit upgrade 1.0.0
```

To downgrade back to 0.x, run

```bash
$ boschdoit upgrade 0.15.31
```
