---
title: Server
description: Interact with boschdoit server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `boschdoit serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an boschdoit client can use.
...
```bash
boschdoit serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
boschdoit serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `BOSCHDOIT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `boschdoit`, or set `BOSCHDOIT_SERVER_USERNAME` to override it. This applies to both `boschdoit serve` and `boschdoit web`.

```bash
BOSCHDOIT_SERVER_PASSWORD=your-password boschdoit serve
```
...
When you run `boschdoit` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the boschdoit server to interact with boschdoit programmatically.
:::

This architecture lets boschdoit support multiple clients and allows you to interact with boschdoit programmatically.

You can run `boschdoit serve` to start a standalone server. If you have the
boschdoit TUI running, `boschdoit serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the BoschDoIt [IDE](/docs/ide) plugins.
...
The boschdoit server exposes the following APIs.
