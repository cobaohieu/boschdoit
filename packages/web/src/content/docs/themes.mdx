...
With BoschDoIt you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, BoschDoIt uses our own `boschdoit` theme.
...
BoschDoIt comes with several built-in themes.
...
- Want BoschDoIt to match their terminal's appearance
...
BoschDoIt supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/boschdoit/themes/*.json` or `$XDG_CONFIG_HOME/boschdoit/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.boschdoit/themes/*.json`
4. **Current working directory** - Defined in `./.boschdoit/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/boschdoit/themes
vim ~/.config/boschdoit/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .boschdoit/themes
vim .boschdoit/themes/my-theme.json
```
...
{
  "$schema": "https://boschdoit.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
