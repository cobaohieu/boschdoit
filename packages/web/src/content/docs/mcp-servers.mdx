...
You can add external tools to BoschDoIt using the _Model Context Protocol_, or MCP. BoschDoIt supports both local and remote servers.
...
You can define MCP servers in your [BoschDoIt Config](https://boschdoit.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="boschdoit.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/boschdoit` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
```jsonc title="boschdoit.jsonc" {15}
{
  "$schema": "https://boschdoit.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
BoschDoIt automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, BoschDoIt will:
...
If the server requires authentication, BoschDoIt will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `boschdoit mcp auth <server-name>`.
...
```json title="boschdoit.json" {7-11}
{
  "$schema": "https://boschdoit.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
boschdoit mcp auth my-oauth-server
```
...
```bash
boschdoit mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, BoschDoIt will store the tokens securely in `~/.local/share/boschdoit/mcp-auth.json`.
...
boschdoit mcp debug my-oauth-server
```
...
Your MCPs are available as tools in BoschDoIt, alongside built-in tools. So you can manage them through the BoschDoIt config like any other tool.
...
```json title="boschdoit.json" {11, 14-18}
{
  "$schema": "https://boschdoit.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
boschdoit mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect BoschDoIt to your Sentry account.
