...
:::caution
Modes are now configured through the `agent` option in the boschdoit config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in boschdoit allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the boschdoit config.
...
boschdoit comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.boschdoit/plans/*.md` to detail the plan itself
...
Configure modes in your `boschdoit.json` config file:
...
- Global: `~/.config/boschdoit/modes/`
- Project: `.boschdoit/modes/`

```markdown title="~/.config/boschdoit/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global boschdoit config and the project specific config.
...
```json title="boschdoit.json" {4-14}
{
  "$schema": "https://boschdoit.ai/config.json",
  "mode": {
    "docs": {
...
Create mode files in `.boschdoit/modes/` for project-specific modes or `~/.config/boschdoit/modes/` for global modes:

```markdown title=".boschdoit/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/boschdoit/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
