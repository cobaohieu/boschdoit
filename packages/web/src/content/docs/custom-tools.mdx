---
title: Custom Tools
description: Create tools the LLM can call in boschdoit.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside boschdoit's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.boschdoit/tools/` directory of your project.
- Or globally, by placing them in `~/.config/boschdoit/tools/`.
...
```ts title=".boschdoit/tools/database.ts" {1}
import { tool } from "@boschdoit-ai/plugin"
...
```ts title=".boschdoit/tools/math.ts"
import { tool } from "@boschdoit-ai/plugin"
...
```ts title=".boschdoit/tools/project.ts" {8}
import { tool } from "@boschdoit-ai/plugin"
...
```python title=".boschdoit/tools/add.py"
import sys
...
```ts title=".boschdoit/tools/python-add.ts" {10}
import { tool } from "@boschdoit-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".boschdoit/tools/add.py")
