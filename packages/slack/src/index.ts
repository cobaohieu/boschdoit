...
import { createBoschdoit, type ToolPart } from "@boschdoit-ai/sdk"
...
console.log("🚀 Starting boschdoit server...")
const boschdoit = await createBoschdoit({
  port: 0,
})
console.log("✅ Boschdoit server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await boschdoit.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new boschdoit session...")
    const { client, server } = boschdoit
    ...
    console.log("✅ Created boschdoit session:", createResult.data.id)
  ...
  console.log("📝 Sending to boschdoit:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Boschdoit response:", JSON.stringify(result, null, 2))
